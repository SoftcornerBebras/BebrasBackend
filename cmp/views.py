...
class CustomizePPT(APIView):  # Get functions related to ppt to zip
    ...
    def ppt(self, f, data, school, type, duplicate):
        ...
        while i < (len(data) - 1):
            ...
            p = ''
            if (type == 'participation'):
                path = os.path.join(settings.MEDIA_ROOT) + '/output//' + data[i]['Name'] + '-' + data[i][
                    'loginID'] + '-' + school + '-Class-' + data[0]['class'] + '-' + data[0]['group'] + '-' + data[0][
                           'year'] + '.pptx'
                p = school + '-Class-' + data[0]['class'] + '-' + data[0]['group'] + '-' + data[0]['year']
                fantasy_zip = zipfile.ZipFile(
                    os.path.join(settings.MEDIA_ROOT) + ("/output/") + school + '-Class-' + data[0]['class'] + '-' +
                    data[0]['year'] + '.zip', 'w')
            elif (type == 'schoolToppers'):
                path = os.path.join(settings.MEDIA_ROOT) + '/output//' + data[i]['Name'] + '-' + data[i][
                    'loginID'] + '-' + school + '-Toppers-' + data[0]['group'] + '-' + data[0]['year'] + '.pptx'
                fantasy_zip = zipfile.ZipFile(
                    os.path.join(settings.MEDIA_ROOT) + ("/output/") + school + '-Toppers-' + data[0]['group'] + '-' +
                    data[0]['year'] + '.zip', 'w')
                p = school + '-Toppers-' + data[0]['group'] + '-' + data[0]['year']
            elif (type == 'nationalToppers'):
                path = os.path.join(settings.MEDIA_ROOT) + '/output//' + data[i]['Name'] + '-' + data[i][
                    'loginID'] + '-' + 'National Toppers-' + data[0]['group'] + '-' + data[0]['year'] + '.pptx'
                fantasy_zip = zipfile.ZipFile(
                    os.path.join(settings.MEDIA_ROOT) + ("/output/") + 'National Toppers-' + data[0]['group'] + '-' +
                    data[0]['year'] + '.zip', 'w')
        ...
        for folder, subfolders, files in walk(os.path.join(settings.MEDIA_ROOT) + ("/output/")):
            for file in files:
                if file.endswith('.pdf') and file.find(p) != -1:
                    print('Writing in zip')
                    fantasy_zip.write(os.path.join(folder, file),
                                      os.path.relpath(os.path.join(folder, file),
                                                      os.path.join(settings.MEDIA_ROOT) + ("/output/")),
    ...
    def get(self, request, **kwargs):
        try:
            ...
            if (type == 'participation'):
                ...
                os.remove(os.path.join(settings.MEDIA_ROOT) + '/output//' + sch.data[0]['schoolName'] + ', ' +
                          sch.data[0]['addressID']['city'] + '-Class-' + str(Class) + '-' + year + '.zip')
                print('removed zip file')
            if (type == 'schoolToppers'):
                School = school.objects.filter(schoolID=id)
                sch = SchoolSerializers(School, many=True)
                os.remove(os.path.join(settings.MEDIA_ROOT) + '/output//' + sch.data[0]['schoolName'] + ', ' +
                          sch.data[0]['addressID']['city'] + '-Toppers-' + group + '-' + year + '.zip')
            if (type == 'nationalToppers'):
                os.remove(
                    os.path.join(settings.MEDIA_ROOT) + '/output//' + 'National Toppers-' + group + '-' + year + '.zip')
            return Response(status=200)
        except Exception as e:
            return Response(e, status=500)
...
class GetParticipationCertificates(APIView):  # Get Participation Certificate API
    authentication_classes = (TokenAuthentication,)
    permission_classes = (permissions.IsAuthenticated,)
    print('in getParticipationCertificate')
    def get(self, request, **kwargs):
        try:
            print('in try of getParticipationCertificate')
            c = CustomizePPT()
            ...
            if len(d.data) > 0:
                ...
                for i in range(0, len(d.data)):
                    for j in range(0, len(agegrpid.data)):
                        if d.data[i]['competitionAgeID']['AgeGroupClassID']['AgeGroupID']['AgeGroupID'] == \
                                agegrpid.data[j]['AgeGroupID']:
                            tot = GetTotalMarks()
                    ...
                    data.append({'year': d.data[i]['competitionAgeID']['competitionID']['startDate'][0:4],
                                 ...
                                 'group': d.data[i]['competitionAgeID']['AgeGroupClassID']['AgeGroupID'][
                                     'AgeGroupName'],
                                 'score': str(d.data[i]['score'] + d.data[i]['bonusMarks']),
                                 'total': totalMarks.data,
                                 'class': str(d.data[i]['schoolClassID']['classNumber'])})
                school = d.data[0]['schoolClassID']['schoolID']['schoolName'] + ', ' + \
                         d.data[0]['schoolClassID']['schoolID']['addressID']['city']
                type = 'participation'
                print('calling def ppt')
            ...
            if len(d.data) > 0:
                ...
                if len(data) > 0:
                    ...
                    school = d.data[0]['schoolClassID']['schoolID']['schoolName'] + ', ' + \
                             d.data[0]['schoolClassID']['schoolID']['addressID']['city']
                    type = 'schoolToppers'
                    c.ppt(os.path.join(settings.MEDIA_ROOT) + '/ppt//' + template.data, data, school, type,
                          duplicate=False)
                    return Response(status=200)
                else:
                    return Response(status=204)
...
class GetStateWiseStudents(APIView):  # Get Toppers StateWise API
    ...
    def get(self, request, **kwargs):
        try:
            ...
            stu = []
            for i in range(0, len(students)):
                if (students[i].schoolClassID.schoolID.addressID.countryID.countryID == int(kwargs['countryID']) and
                        students[i].schoolClassID.schoolID.addressID.stateID.stateID == int(kwargs['stateID'])
                        and students[i].competitionAgeID.competitionID.competitionID == int(kwargs['cmpID'])
                        and students[i].competitionAgeID.AgeGroupClassID.AgeGroupID.AgeGroupID == int(kwargs['AgeID'])):
                    stu.append(students[i].studentEnrollmentID)
            lists = studentEnrollment.objects.filter(studentEnrollmentID__in=stu)
    ...
    def get(self, request, **kwargs):
        try:
            ...
            for i in range(0, len(students)):
                if (students[i].schoolClassID.schoolID.addressID.countryID.countryID == int(kwargs['countryID']) and
                        students[i].schoolClassID.schoolID.addressID.stateID.stateID == int(kwargs['stateID'])
                        and students[i].schoolClassID.schoolID.addressID.districtID.districtID == int(
                            kwargs['districtID'])
                        and students[i].competitionAgeID.competitionID.competitionID == int(kwargs['cmpID'])
                        and students[i].competitionAgeID.AgeGroupClassID.AgeGroupID.AgeGroupID == int(kwargs['AgeID'])):
                    stu.append(students[i].studentEnrollmentID)
...
class GetAllRegisteredStudentsSchoolWise(APIView):
    authentication_classes = (TokenAuthentication,)
    permission_classes = (permissions.IsAuthenticated,)

    def get(self, request, **kwargs):
        try:
            Coordinators = UserRoleLocation.objects.filter(locationObjectID=kwargs['schoolID'],
                                                           locationTypeCodeID='109001')
            CoordinatorID = []
            for i in range(0, len(Coordinators)):
                CoordinatorID.append(Coordinators[i].userRoleID.userRoleID)
            CoordinatorsRoleID = UserRole.objects.filter(userRoleID__in=CoordinatorID)
            s = []
            print(CoordinatorsRoleID)
            for i in range(0, len(CoordinatorsRoleID)):
                obj = User.objects.filter(created_by=CoordinatorsRoleID[i].userID.loginID)
                for j in range(0, len(obj)):
                    s.append(obj[j].userID)
            RoleID = Role.objects.get(RoleName='Student')
            students = UserRole.objects.filter(userID__in=s, RoleID=RoleID).order_by('-userRoleID')
            paginator = CustomPagination()
            response = paginator.generate_response(students, UserRoleSerializer, request)
            return Response({
                "StudData": response.data,
            })
        except Exception as e:
...
class RollbackQuestion(APIView):  # Rollback process for QuesInsert API
    ...
    def get(self, requests):
        try:
            ...
            if questionTranslation.objects.filter(
                    questionID=QuesID.questionID).exists() == True and correctOption.objects.filter(
                    questionTranslationID=questionTranslation.objects.filter(questionID=QuesID.questionID)[
                        0].questionTranslationID).exists() == True:
                return Response(status=200)
            else:
                if questionTranslation.objects.filter(questionID=QuesID.questionID).exists() == False:
                    ...
                    if questionTranslation.objects.filter(
                            questionID=QuesID.questionID).exists() == True and correctOption.objects.filter(
                            questionTranslationID=questionTranslation.objects.filter(questionID=QuesID.questionID)[
                                0].questionTranslationID).exists() == False:
                        if Image.objects.filter(ObjectID=QuesID.questionID, ImageTypeCodeID=code.objects.get(
                                codeName='ImageQuestion').codeID).exists() == True:
                            imglist = Image.objects.filter(ObjectID=QuesID.questionID, ImageTypeCodeID=code.objects.get(
                                codeName='ImageQuestion').codeID)
                        ...
                        if option.objects.filter(questionID=QuesID.questionID).exists() == True:
                            ...
                            for i in range(0, len(optlist)):
                                ...
                                if Image.objects.filter(ObjectID=optlist[i].optionID, ImageTypeCodeID=code.objects.get(
                                        codeName='ImageOption').codeID).exists() == True:
                                    Image.objects.filter(ObjectID=optlist[i].optionID, ImageTypeCodeID=code.objects.get(
                                        codeName='ImageOption').codeID)[0].delete()
                                optlist[i].delete()
                        QuesID.delete()
            return Response(status=200)
        except Exception as e:
